...
void SCR_LoadPCX(char * filename, byte ** pic, byte ** palette, int * width, int * height)
{
    ...
    *pic = out;
    ...
    if (raw - (byte *)pcx > len)
    {
        Com_DPrintf("PCX file %s was malformed\n", filename);
        Z_Free(*pic);
        *pic = NULL;
    }
...
cblock_t Huff1Decompress(cblock_t in)
{
    ...
    {
        Com_DPrintf("Decompression overread by %i\n", (input - in.data) - in.count);
    }
...
byte * SCR_ReadNextFrame(void)
{
    ...
    if (r != 1)
    {
        Com_DPrintf("Failed to fread() cinematic frame!\n");
        return NULL;
    }
    ...
    if (command == 2)
    {
        Com_DPrintf("Cinematic hit last frame!\n");
...
Returns true if a cinematic is active, meaning
the view rendering should be skipped.
==================
*/
...
==================
*/
static char last_test_cinematic[MAX_QPATH];
qboolean CinematicTest_PlayDirect(const char * filename)
{
    ...
    {
        Com_DPrintf("Cinematic '%s' can't be opened!\n", filename);
        cl.cinematictime = 0;
        return false;
    }
...
qboolean CinematicTest_RunFrame(void)
{
    ...
    {
        Com_DPrintf("Stopping cinematic '%s'\n", last_test_cinematic);
        SCR_StopCinematic(); // Frees memory and closes the file handle.
        return false;
    }
