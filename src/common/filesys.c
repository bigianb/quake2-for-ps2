...
The sys_* files pass this to host_init in quakeparms_t->basedir.  This can be overridden with
the "-basedir" command line parm to allow code debugging in a different directory.
...
files (savegames, screenshots, demos, config files) will be saved to. This can be overridden
...
int FS_FOpenFile(const char * filename, FILE ** file)
{
    ...
    if (!strcmp(filename, "config.cfg") || !strncmp(filename, "players/", 8))
    {
        ...
        *file = fopen(netpath, "rb");
        ...
        Com_DPrintf("FS_FOpenFile (NO_ADDONS): %s\n", netpath);
...
void FS_AddGameDirectory(char * dir)
{
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        Com_DPrintf("FS_AddGameDirectory added pak '%s'\n", pakfile);
...
void FS_InitFilesystem(void)
{
    ...
    //
    fs_basedir = Cvar_Get("basedir", fs_default_base_path, CVAR_NOSET);
    ...
    Com_DPrintf("fs_gamedir: %s\n", fs_gamedir);
    Com_DPrintf("fs_basedir: %s\n", fs_basedir->string);
    Com_DPrintf("fs_cddir:   %s\n", fs_cddir->string);

    Com_DPrintf("---- FS_InitFilesystem completed! ----\n");
}
