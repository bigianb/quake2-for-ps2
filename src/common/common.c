...
int MSG_ReadLong(sizebuf_t * msg_read)
{
    ...
    if (msg_read->readcount + 4 > msg_read->cursize)
        ...
        c = msg_read->data[msg_read->readcount] + (msg_read->data[msg_read->readcount + 1] << 8) + (msg_read->data[msg_read->readcount + 2] << 16) + (msg_read->data[msg_read->readcount + 3] << 24);
...
void Z_Free(void * ptr)
{
    ...
    if (z->magic != Z_MAGIC)
    {
        Sys_Error("Z_Free: Bad magic 0x%X", z->magic);
    }
...
void Qcommon_Init(int argc, char ** argv)
{
    ...
    // this is where pak files are loaded.
    // FS init will attempt to open any paks found in the game dir.
    FS_InitFilesystem();
